...
## 十五、Vue路由销毁问题
> Vue在路由切换的时候会销毁旧路由并重新加载
> 
> 因此，如果放在组件中的请求，在复用时会多次发送，如果该请求数据不需要多次更新，可以通过以下方式优化
> 
> 我们可以在App的根组件的 `mounted` 中进行请求，并储存到 `$store` 中，这样只会执行一次请求


## 十六、组件间通信
> 组件之间的通信有十几种方法，基本的有以下六种
> - `props` : 用于父给子传递信息
> - `@on` `@emit` : 自定义事件，用于子给父传递信息
> - `$bus` : 全局事件总线，全能
> - `pubsub.js` : 消息订阅发布，全能
